...
public class MarinaWebServiceImpl implements MarinaWebServiceInterface {
    ...
    try {
        ...
        Connection conn = DriverManager.getConnection("jdbc:mysql://sql9.freemysqlhosting.net:3306/sql9134943","sql9134943","lhnt9n8tY2");
        ...
        reply = stmt.executeQuery("SELECT firstName, lastName, boatType, length, year, motorType, feePaid FROM sailboats " +
            "WHERE firstName = '" + firstName + "' AND lastName = '" + lastName + "' AND boatType = '" + boatType + "' AND length = '" + length + "' AND year =  '" + year + "' AND motorType = '" + motorType + "'");
        if( reply.next() ) {
            String result = " First Name: " + reply.getString("firstName");
            ...
            if(reply.getString("feePaid").equals("n")) result = "FEE NOT PAID. " + result; 
		...
		return "No matching sailor found.";
	}
}